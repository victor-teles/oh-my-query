...
macro_rules! fetch_rows_native {
    ($pool:expr, $sql:expr, $max_rows:expr) => {{
        ...
        let mut columns: Option<Vec<$crate::types::ColumnInfo>> = None;
            ...
            .map_err($crate::error::DbError::from)?
        {
            if columns.is_none() {
                ...
                for col in cols {
                    col_info.push($crate::types::ColumnInfo {
        ...
        Ok::<_, $crate::error::DbError>((columns.unwrap_or_default(), rows, is_truncated))
    }};
}
