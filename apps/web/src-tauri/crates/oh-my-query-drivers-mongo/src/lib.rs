...
use oh_my_query_core::types::{
    ColumnInfo, ConnectionParams, ExecuteResult, SchemaInfo, SchemaItem, TableItem,
...
impl Pool for MongoDbPool {
    ...
    async fn fetch_schema(&self, database: &str) -> Result<SchemaInfo, DbError> {
        fetch_schema_mongodb(&self.client, database).await
    }
    ...
    async fn execute(
        ...
        execute_mongodb(&self.client, command, max_rows).await
    }
...
pub async fn fetch_schema_mongodb(
